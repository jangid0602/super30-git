...
Approach:
We maintain:
1) An integer stack[] to store values.
2) A double avgStack[] to store average till each index.

At every push:
newAvg = (previousAvg * previousSize + newValue) / newSize

So getAverage() becomes O(1).
...
Approach:
- Traverse both lists
- Extract k nodes from each list
- Rearrange pointers accordingly
- No new nodes created (in-place merging)
...
- If size == capacity → dequeue first
...
Approach:
Repeatedly traverse list:
If current.val == next.val → merge counts and remove next.
...
MAIN CLASS: Week3Story

Menu-driven program to run all problems.
